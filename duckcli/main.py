...
from .lexer import DuckCliLexer
...
class DuckCli(object):
    ...
    def __init__(
        ...
        # Load config.
        c = self.config = get_config(duckclirc)
    ...
    def initialize_logging(self):
        ...
        root_logger = logging.getLogger("duckcli")
        root_logger.addHandler(handler)
        root_logger.setLevel(level_map[log_level.upper()])

        logging.captureWarnings(True)

        root_logger.debug("Initializing duckcli logging.")
    ...
    def run_cli(self):
        ...
        with self._completer_lock:
            ...
            self.prompt_app = PromptSession(
                lexer=PygmentsLexer(DuckCliLexer),
...
@click.command()
@click.option("-V", "--version", is_flag=True, help="Output duckcli's version.")
@click.option("-D", "--database", "dbname", help="Database to use.")
@click.option(
    "-R",
    "--prompt",
    "prompt",
    help='Prompt format (Default: "{0}").'.format(DuckCli.default_prompt),
)
@click.option(
    "-l",
    "--logfile",
...
@click.option(
    ...
    help="Location of duckclirc file.",
...
def cli(
    ...
    duckcli.connect(database)

    duckcli.logger.debug("Launch Params: \n" "\tdatabase: %r", database)

    #  --execute argument
    if execute:
        try:
            if csv:
                duckcli.formatter.format_name = "csv"
            elif not table:
                duckcli.formatter.format_name = "tsv"

            duckcli.run_query(execute)
            exit(0)
        except Exception as e:
    ...
    if sys.stdin.isatty():
        ...
        try:
            ...
            duckcli.logger.warning("Unable to open TTY as stdin.")

        if (
            duckcli.destructive_warning
        ...
        try:
            ...
            duckcli.run_query(stdin_text, new_line=new_line)
            exit(0)
        except Exception as e:
            click.secho(str(e), err=True, fg="red")
            exit(1)
